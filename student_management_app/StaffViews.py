...
def staff_taken_attendance(request):
    ...
    return render(request, "staff_template/staff_taken_attendance.html",
...
@csrf_exempt
def save_updateattendance_date(request):
    student_ids = request.POST.get("student_ids")
    attendance_date = request.POST.get("attendance_date")
    attendance = Attendance.objects.get(id=attendance_date)
...
def staff_all_notification(request):
    staff = Staffs.objects.get(admin=request.user.id)
    notification = NotificationStaffs.objects.filter(staff_id=staff.id)
    return render(request, "staff_template/all_notification.html", {"notification": notification})
